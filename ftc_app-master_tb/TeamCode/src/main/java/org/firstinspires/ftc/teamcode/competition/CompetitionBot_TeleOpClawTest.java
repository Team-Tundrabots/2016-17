...
@TeleOp(name="CompetitionBot: TeleOpClawTest", group="competition")
//@Disabled

public class CompetitionBot_TeleOpClawTest extends OpMode{
    ...
    public void loop() {
        ...
        //use x an y to open and close top claw
        if (gamepad1.y) {
            clawTopOffset += CLAW_SPEED;
            robot.moveTopClaws(clawTopOffset);
        } else if (gamepad1.x) {
            clawTopOffset -= CLAW_SPEED;
            robot.moveTopClaws(clawTopOffset);
        }
